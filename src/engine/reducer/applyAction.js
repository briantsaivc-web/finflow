...
E.apply = function(state, action, opts){
  ...
  case "REPAY_LOAN": {
    ...
    // S42：金額必須是有限正數。NaN／undefined（JSON 丟欄位）會讓 Math.min 回 NaN、
    // 繞過 <=0 守門，現金與本金從此全是 NaN。
    var reqAmt = action.payload ? action.payload.amount : undefined;
    if(typeof reqAmt!=="number" || !isFinite(reqAmt) || reqAmt<=0) return reject("BAD_AMOUNT");
    var amt = Math.min(reqAmt, l.principal, actor.cash);
  ...
  case "TAKE_LOAN": {
    ...
    var reqAmt2 = action.payload ? action.payload.amount : undefined;   // S42：同 REPAY_LOAN
    if(typeof reqAmt2!=="number" || !isFinite(reqAmt2) || reqAmt2<=0) return reject("BAD_AMOUNT");
    var amt2 = Math.min(reqAmt2, cap);
  ...
  case "BUY_DREAM_PROGRESS": {
    ...
    if(p.cash < E.dreamProgressPrice(S,p)) return reject("NO_CASH");  // S42：先驗資再 accept，不留無效紀錄（價格含技能折抵，與扣款同一份）
    accept();
...
E.buyAsset = function(S,p,card,optionId,params){
  ...
  if(card.kind==="REALESTATE"){
    ...
    var ltv = useLoan ? Math.min(params.ltv!==undefined?params.ltv:(1-(pl.downPayment||0)/(pl.price||1)), E.effMaxLTV(S,card)) : 0;   // S42：對齊 engineCore E.oppCompare 的除零防禦
...
E.enterBankruptcy = function(S,p){
  /* S42：引擎不變式「待決策一定屬於當前玩家」（見 2730／2925 註解）。期貨逐輪結算、
     下市歸零都跑在別人的回合、遍歷全體玩家——非回合真人在這裡被推進破產（或外圈跌落）
     決策，會被 S14a-2 白名單擋住急售／紓困（NOT_YOUR_TURN），當前玩家也擲不了骰、
     結束不了回合，只剩「退出這一局」。與 queueFutNotice／markDelistWarn 同一紀律：
     先記在玩家身上，等 E.beginTurn 輪到他再推。電腦玩家走 npcRescue 當場處理，不受影響。 */
  if(!p.isNPC && !p.bankrupt && p!==E.activePlayer(S)){ p.pendingBankruptcy=true; return; }
...
E.beginTurn = function(S){
  ...
  // S42：別人回合裡被期貨強平／下市打到現金負的真人，破產程序遞延到這裡才推（見 enterBankruptcy）。
  // 發薪與上面幾個結算跑完若已回正，就不推了（現金 >= 0 不是破產）。
  if(p.pendingBankruptcy){ delete p.pendingBankruptcy; if(p.cash<0 && !p.bankrupt) E.enterBankruptcy(S,p); }
  // S22：上面兩個結算可能把人打到現金負——電腦玩家走 npcRescue 失敗就地破產、真人則排入破產決策。
  // 與 S14b 發薪那段同一個洞：破產已成定局卻停在 ROLL，誰都推不動。這裡照同一套收尾。
  if(p.bankrupt){ S.turnResolved=true; return E.endTurn(S); }
  if((S.decisionQueue||[]).some(function(d){ return d.kind==="BANKRUPTCY" || d.kind==="FREEFALL_RESCUE"; })){ E.syncPhase(S); return; }
